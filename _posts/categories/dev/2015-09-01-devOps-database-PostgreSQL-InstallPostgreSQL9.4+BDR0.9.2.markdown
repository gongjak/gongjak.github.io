...
subtitle:   "Install PostgreSQL 9.4 + BDR 0.9.2"
categories: devops
...
- 목차
	...
	- [1. 패키지 설치 작업](#패키지-설치-작업)
	- [2. 소스 다운로드](#소스-다운로드)
	- [3. database 초기화](#database-초기화)
	- [4. Database 서비스 계정 추가](#database-서비스-계정-추가)
	- [5. postgresql.conf 설정](#postgresql-config-설정)
	- [6. pg_hba.conf 설정](#pg_hba-config-설정)
	- [7. PostgreSQL 기동 절차](#postgresql-기동-절차)
	- [8. Database 생성](#database-생성)
	- [9. 각 노드 구성](#각-노드-구성)
	- [10. 노드 제거](#노드-제거)
	- [11. 노드 추가](#노드-추가)
	- [12. bdr 모니터링](#bdr-모니터링)
	- [13. 백업 관련](#백업-관련)
	- [14. 파티션 테이블 관리](#파티션-테이블-관리)
	- [15. 주의 사항](#주의-사항)
...
## 소스 다운로드
http://bdr-project.org/docs/0.9.0/installation-source.html
...
- 확인하는 쿼리(bdr.bdr_node_join_wait_for_ready)를 이용한 확인에는 시간이 걸릴수도 있다. 모든 설정이 정상이라면 기다리면 결과가 나온다.만약 기다려도 답이 안올 경우, postgresql.conf 파일 중 max_wal_senders 와 max_replication_slots 값과 전체 노드 수가 맞는지 확인한다.
	...
	### 1) asia 1번 노드에 BDR 적용 - 사실 모든 서버에 이걸 해 줘야 BDR 사용이 가능함
	...
	- 메인 노드 그룹 생성
	    ...
	    ```sql
	    SELECT bdr.bdr_group_create(local_node_name := 'asia-node-001',node_external_dsn := 'host=asia-psql-bdr-001 port=5432 dbname=testdb');
	    ```
	...
	- 메인 그룹 참여
	    ...
	    ```sql
	    SELECT bdr.bdr_group_join(local_node_name := 'asia-node-002',node_external_dsn := 'host=asia-psql-bdr-002 port=5432 dbname=testdb',join_using_dsn := 'host=asia-psql-bdr-001 port=5432 dbname=testdb');
	    ```
	...
	- 메인 그룹 참여
	    ...
	    ```sql
	    SELECT bdr.bdr_group_join(local_node_name := 'us-node-001',node_external_dsn := 'host=us-psql-bdr-001 port=5432 dbname=testdb',join_using_dsn := 'host=asia-psql-bdr-001 port=5432 dbname=testdb');
	    ```
	...
	- 메인 그룹 참여
	    ...
	    ```sql
	    SELECT bdr.bdr_group_join(local_node_name := 'us-node-002',node_external_dsn := 'host=us-psql-bdr-002 port=5432 dbname=testdb',join_using_dsn := 'host=us-psql-bdr-001 port=5432 dbname=testdb');
	    ```
	...
	- 메인 그룹 참여
	    ...
	    ```sql
	    SELECT bdr.bdr_group_join(local_node_name := 'europe-node-001',node_external_dsn := 'host=europe-psql-bdr-001 port=5432 dbname=testdb',join_using_dsn := 'host=asia-psql-bdr-001 port=5432 dbname=testdb');
	    ```
	...
	- 메인 그룹 참여
	    ...
	    ```sql
	    SELECT bdr.bdr_group_join(local_node_name := 'europe-node-003',node_external_dsn := 'host=europe-psql-bdr-002 port=5432 dbname=testdb',join_using_dsn := 'host=europe-psql-bdr-001 port=5432 dbname=testdb');
	    ```
...
## bdr 모니터링
	...
	SELECT pg_xlog_location_diff(pg_current_xlog_insert_location(), flush_location) AS lag_bytes, pid, application_name
...
2. Global 시퀀스를 사용해야 함
    ...
    http://bdr-project.org/docs/0.9.0/global-sequences.html
...
3. ddl 문 실행시 제약 조건
    ...
    http://bdr-project.org/docs/0.9.0/ddl-replication.html
